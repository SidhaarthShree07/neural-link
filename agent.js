/**
 * NeuraLink - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Sidhaarth Shree (Original) & Assistant (fixes)
 */
class NeuraLink {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`NeuraLink v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize NeuraLink:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'NeuraLink', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to NeuraLink! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new NeuraLink();
});
